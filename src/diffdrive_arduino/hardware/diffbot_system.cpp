...
//Added
  // Create a standalone ROS node
  ros_node_ = rclcpp::Node::make_shared("diffdrive_arduino_custom_comm");

  // Create executor to spin this node independently
  executor_ = std::make_shared<rclcpp::executors::SingleThreadedExecutor>();
  executor_->add_node(ros_node_);

  // Create publisher for Arduino response
  serial_response_pub_ = ros_node_->create_publisher<std_msgs::msg::String>("/arduino_serial_response", 10);

  // Subscribe to custom string topic
  custom_sub_ = ros_node_->create_subscription<std_msgs::msg::String>(
    "/custom_serial_topic", 10,
    [this](const std_msgs::msg::String::SharedPtr msg)
    {
      if (comms_.connected())
      {
        std::string response = comms_.send_msg(msg->data + "\r", false);
      
        std_msgs::msg::String response_msg;
        response_msg.data = response;
        serial_response_pub_->publish(response_msg);

        std::string full_log = "Sent to Arduino: '" + msg->data + "', Received: " + response;
        RCLCPP_INFO(ros_node_->get_logger(), "%s", full_log.c_str());
      }
      else
      {
        RCLCPP_WARN(ros_node_->get_logger(), "Serial not connected, message not sent.");
      }
    }
  );
//Added
